...
import { Mail, Lock, User as UserIcon, ArrowRight, Loader2, Eye, EyeOff, CheckCircle } from 'lucide-react';
...
const Signup: React.FC = () => {
  ...
  const [oauthButtonWidth, setOauthButtonWidth] = useState(384);
    ...
    <GoogleOAuthProvider clientId={GOOGLE_CLIENT_ID}>
      <div className="container" style={{ display: 'flex', justifyContent: 'center', padding: '4rem 1.5rem' }}>
        <motion.div
          ...
          className="card"
          style={{ width: '100%', maxWidth: '480px', padding: '3rem', position: 'relative', overflow: 'hidden' }}
        >
          <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
            ...
            <p style={{ color: 'var(--text-muted)' }}>
              {step === 1 ? 'Get your premium temporary inbox in seconds.' : `We sent an OTP to ${email}`}
            </p>
          </div>
            ...
            {step === 1 ? (
              ...
              >
                ...
                <p style={{ textAlign: 'center', marginTop: '1rem', color: 'var(--text-muted)', fontSize: '0.85rem' }}>
                  <button onClick={() => setStep(1)} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', textDecoration: 'underline', cursor: 'pointer', padding: 0 }}>Back to signup</button>
                </p>
              </motion.div>
            )}
          </AnimatePresence>
